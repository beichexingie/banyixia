...
-- Service pricing runtime fields. Legacy day price/enabled columns remain for
-- old rows, but new orders use service type + hourly price only.
...
update public.guide_service_items
set service_type = name
...
alter table if exists public.orders
  add column if not exists service_item_id uuid references public.guide_service_items(id) on delete set null,
  add column if not exists service_hours numeric(8,2),
  add column if not exists service_fee numeric(12,2),
  add column if not exists travel_fee numeric(12,2) not null default 0,
  add column if not exists platform_fee numeric(12,2) not null default 0,
  add column if not exists guide_income numeric(12,2) not null default 0,
  add column if not exists guide_service_lat double precision,
  add column if not exists guide_service_lng double precision;

create index if not exists idx_guide_availability_rules
  on public.guide_availability (guide_id, recurrence_type, date_start, date_end);
