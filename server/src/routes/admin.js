...
async function updateContentReview(req, res, status) {
  ...
  if (mapped.label === 'guide_application') {
    ...
    if (status === 'approved' && result.rows[0]) {
      // The database trigger sync_guide_on_approval is the single writer for
      // the guides projection. Keeping this route free of a second upsert
      // prevents application and guide data from drifting apart.
      await pool.query(
